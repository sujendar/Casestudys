package com.pixelTrice.elastic;


import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;

import java.io.IOException;
import java.util.Iterator;
import java.util.List;

@RestController
public class ElasticSearchController {

    @Autowired
    private ElasticSearchQuery elasticSearchQuery;

    @PostMapping("/createOrUpdateDocument")
    public ResponseEntity<Object> createOrUpdateDocument(@RequestBody Product product) throws IOException {
    	String response=null;
    	for (int i = 0; i < 5; i++) {
    		product.setId("BOOK-"+i);
    		 response = elasticSearchQuery.createOrUpdateDocument(product);
		}
         
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    @GetMapping("/getDocument")
    public ResponseEntity<Object> getDocumentById(@RequestParam String productId) throws IOException {
       Product product =  elasticSearchQuery.getDocumentById(productId);
        return new ResponseEntity<>(product, HttpStatus.OK);
    }

    @DeleteMapping("/deleteDocument")
    public ResponseEntity<Object> deleteDocumentById(@RequestParam String productId) throws IOException {
        String response =  elasticSearchQuery.deleteDocumentById(productId);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    @GetMapping("/searchDocument")
    public ResponseEntity<Object> searchAllDocument(@RequestParam String destination) throws IOException {
    	ResortResponse products = elasticSearchQuery.searchAllDocuments(destination);
        return new ResponseEntity<>(products, HttpStatus.OK);
    }
    @PostMapping("/searchByFilter")
    public ResponseEntity<Object> searchAllDocument(@RequestBody FilterRequest filterRequest) throws IOException {
    	ResortResponse products = elasticSearchQuery.searchByFilter(filterRequest);
        return new ResponseEntity<>(products, HttpStatus.OK);
    }
}
